#!/usr/bin/env bash
# MATH-Level345, homo_qwen25_3b, Qwen2.5-1.5B x Qwen2.5-1.5B, Co-RL (Same family)
...
# smoke: MAX_STEPS=1 bash examples/homo_qwen25_3b.sh
set -euo pipefail
...
MODEL_B="${MODEL_B:-Qwen/Qwen2.5-1.5B}"
VLLM_MEM="${VLLM_MEM:-0.45}"
TS="$(date +%Y%m%d_%H%M%S)"; RUN="same_family_${TS}"
...
launch_group A "0,1" "$MODEL_A" "$MODEL_B" 20421 "$BASE_OUT/model_a" & PID_A=$!
launch_group B "2,3" "$MODEL_B" "$MODEL_A" 20422 "$BASE_OUT/model_b" & PID_B=$!
